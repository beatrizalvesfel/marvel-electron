...
const CharacterDetails = () => {
  ...
  if (loading) return <p className="text-center text-lg font-semibold dark:text-white">Carregando detalhes...</p>;
  if (error) return <p className="text-center text-red-500 dark:text-red-400">Erro ao carregar personagem.</p>;
  if (!data || data.length === 0) return <p className="text-center dark:text-white">Personagem não encontrado.</p>;

  const character = data[0];

  if (!character || !character.thumbnail) {
    return <p className="text-center dark:text-white">Dados do personagem incompletos.</p>;
  }

  return (
    <div className="flex flex-col items-center p-6 bg-gray-100 dark:bg-gray-900 min-h-screen">
      <div className="w-full max-w-4xl">
        <Link to="/" className="flex items-center text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-500 transition mb-4">
          <ArrowLeft className="w-6 h-6 mr-2" /> Voltar
        </Link>
      </div>

      <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden w-full max-w-4xl">
        <img
          src={`${character.thumbnail.path}.${character.thumbnail.extension}`}
          alt={character.name}
          className="w-full h-96 object-cover"
